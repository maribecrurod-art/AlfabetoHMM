...
::: {#nte-espacio-filtrado .callout-note title="Definición (Espacio de Probabilidad Filtrado)"}
...
::: {#nte-espacio-estados-discreto .callout-note title="Definición (Espacio de Estados Discreto)"}
...
::: {#nte-proceso-estocastico .callout-note title="Definición (Proceso Estocástico a Tiempo Continuo)"}
...
::: {#nte-propiedad-markov .callout-note title="Definición (Propiedad de Markov)"}
...
::: {#nte-probabilidades-transicion .callout-note title="Definición (Probabilidades de Transición y Homogeneidad)"}
...
::: {#imp-chapman-kolmogorov .callout-important title="Propiedad (Ecuaciones de Chapman-Kolmogorov)"}
...
![La probabilidad de transición $p\_{ij}(t+s)$ se obtiene sumando las contribuciones de todos los posibles estados intermedios $k$.](chapman-k.png){#fig-chapman fig-align="center" width="550"}

Por otro lado, también se tiene la existencia de los HMM categóricos.

::: {#nte-hmm-categorico-1 .callout-note title="Definición (Modelo Oculto de Markov Categórico)"}
...
::: {#nte-matriz-generadora .callout-note title="Definición (Matriz Generadora Infinitesimal)"}
...
::: {#imp-ecuaciones-dif-kolmogorov .callout-important title="Propiedad (Ecuaciones Diferenciales de Kolmogorov)"}
...
::: {#nte-proceso-latente .callout-note title="Definición (Proceso Latente e Inobservable)"}
...
::: {#nte-proceso-observacion .callout-note title="Definición (Proceso de Observación Empírica)"}
Sea
...
::: {#nte-matriz-emision .callout-note title="Definición (Matriz de Emisión e Independencia Condicional)"}
...
donde $\mathcal{F}_t^X$ y $\mathcal{F}_{t^-}^Y$ denotan las filtraciones @nte-espacio-filtrado del historial latente y observable, respectivamente.
...
::: {#nte-filtro-forward .callout-note title="Definición (Filtro Hacia Adelante (Forward) y Verosimilitud)"}
...
Esta cantidad se calcula recursivamente mediante la ecuación de Chapman-Kolmogorov @imp-chapman-kolmogorov y la matriz de emisión $E$ @nte-matriz-emision:
...
::: {#nte-decodificacion-optima .callout-note title="Definición (Decodificación Óptima Global)"}
...
A diferencia de la función $\max$, que devuelve el **valor numérico** de la probabilidad más alta alcanzada, el operador $\arg\max$ (argumento del máximo) devuelve el **conjunto de entradas** que producen ese valor máximo. En este contexto, no nos interesa saber cuál es el porcentaje exacto de la probabilidad máxi\
ma, sino descubrir específicamente **cuál es la secuencia exacta de estados latentes** $\hat{X}$ que hace que esa probabilidad sea máxima
...
::: {#nte-baum-welch .callout-important title="Propiedad (Algoritmo de Expectation-Maximization (Baum-Welch))"}
...
![Esquema general del algoritmo Expectation–Maximization (Baum–Welch) para un Modelo Oculto de Markov. En el Paso E se estiman las probabilidades posteriores de los estados ocultos y las transiciones esperadas mediante los algoritmos Forward–Backward. En el Paso M se actualizan iterativamente la matriz generadora y la matriz de emisión para maximizar la verosimilitud de las observaciones.](imagenesHMM/ME_HMM.png){#fig-ME fig-align="center" width="500"}